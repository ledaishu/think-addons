...
/**
 * 获取插件目录
 * @return array 返回所有插件的基础信息
 */
function get_addon_path()
{
	return $addons_path = Env::get('addons_path');
}
function get_app_path()
{
	$appPath = App::getAppPath();
	return dirname($appPath). DIRECTORY_SEPARATOR;//项目跟目录
}
/**
 * 获取插件列表(有优化的空间)
 * @return array 返回所有插件的基础信息
...
function get_addon_list()
{
	...
	foreach ($addons as $v){
        ...
        if(is_file($addons_path . DIRECTORY_SEPARATOR . $v)){
            continue;
        }
        if(is_dir($addons_path . DIRECTORY_SEPARATOR . $v)){
            //判断插件是否为有效插件：是否存在插件类（例如：test插件目录下是否存在Test.php文件，存在即插件有效）
            if(!is_file($addons_path . DIRECTORY_SEPARATOR . $v . DIRECTORY_SEPARATOR .ucfirst($v).'.php')){
                continue;
            }
...
function create_config($config,$name="config.php"){
	$addons_path = Env::get('addons_path');
    $config_file = $addons_path.$config['name']. DIRECTORY_SEPARATOR .$name;
...
function get_addon_test()
{
	$addons_path = Env::get('addons_path');
	$filename = "demo.txt";
	$newname = "new.txt";
	@rename($addons_path.$filename,$addons_path.$newname);
	var_export(get_addon_list());
}
